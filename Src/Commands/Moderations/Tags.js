...
module.exports = class extends Command {
    ...
    constructor(...args) {
        super(...args, {
            description: 'Say a saved message.',
            category: 'Moderation',
    ...
    async run(message) {
        ...
        if (!args[1] || args[1] === "help") return message.channel.send("What do you want to do? `,,tag <create/edit/delete/list/infos/name> <name/text>`")
            ...
            if (!GL[GID].tags[args[2]]) return message.channel.send("There is no tags under that name.")
            let edit = "No edit"
            if (GL[GID].tags[args[2]].edited.number > 0) {
                let thisTag = GL[GID].tags[args[2]]
            ...
            message.channel.send([
                ...
                `**Tag name:** ${args[2]}`,
                `**Author:**: ${GL[GID].tags[args[2]].name}#${GL[GID].tags[args[2]].discriminator} (${GL[GID].tags[args[2]].author})`,
                `**Creation date:** ${GL[GID].tags[args[2]].timestamp} (${ms(Date.now() - GL[GID].tags[args[2]].timestampMS, { long: true })} ago.)`,
                `**Last edit:** ${edit}`
            ].join("\n")).then(msg => {
                msg.react("✅")
                msg.react("❌")
            ...
            message.channel.send("Are you sure that you want to edit this tag?").then(msg => {
                ...
                collector.on('collect', (reaction, user) => {
                    ...
                    if (reaction.emoji.name === "✅") {
                        collector.stop()
                        if (tag.length > 2000) return message.channel.send("For the moment, bots can not send message with more than 2000 letters. Please make your tag shorter.")
                        const editedTag = GL[GID].tags[args[2]]
            ...
            const tagTab = Object.values(GL[GID].tags)
            ...
            if (!GL[GID].tags[args[1]]) return message.channel.send(`No tag created under the name of **${args[1]}**.`)
            else return message.channel.send(GL[GID].tags[args[1]].content).then(message.delete())
        }
    }
}